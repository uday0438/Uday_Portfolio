      ...
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="mt-12 bg-[var(--bg-primary)] p-2 rounded-2xl flex flex-col md:flex-row gap-2 max-w-2xl mx-auto border border-[var(--border-color)]">
          <input
            ...
            className="flex-1 px-6 py-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] outline-none focus:ring-2 focus:ring-blue-400/50 text-[var(--text-primary)] placeholder-[var(--text-muted)] transition-all"
          />
          <input
            type="text"
            value={subject}
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-4">
          ...
          <a
            href={`https://mail.google.com/mail/?view=cm&to=${email}&su=${encodeURIComponent(subject || 'Hello Uday!')}${userEmail ? `&body=${encodeURIComponent(`Sender Email: ${userEmail}`)}` : ''}`}
            target="_blank"
            rel="noopener noreferrer"
            className="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-medium hover:shadow-lg hover:shadow-blue-500/20 hover:scale-105 transition-all flex items-center gap-2"
          >
            Contact Now
          </a>
        </div>
